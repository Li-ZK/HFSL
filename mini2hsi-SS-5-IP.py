...
parser.add_argument("-d","--tar_input_dim",type = int, default = 200) # PaviaU=103；salinas=204； IN=200; KSC=176;Botswana = 144
...
test_IP_data = './datasets/IP/indian_pines_corrected.mat'
test_IP_label = './datasets/IP/indian_pines_gt.mat'
...
torch.save(model.state_dict(), "VGG16-IP.pth")  # 只保存模型的参数
...
class Network(nn.Module):
    def __init__(self, n_bands, n_classes):
        ...
        pretrained_dict = torch.load('./VGG16-IP.pth')
...
seeds = [1330, 1335, 1336, 1337, 1224, 1236, 1226, 1227, 1228, 1229] #
# seeds = [1333, 1335, 1336, 1337, 1224, 1236, 1226, 1235, 1233, 1229]
seeds =  [1336,1224]
def train():
    ...
    for episode in range(1000):  # EPISODE = 500000
        ...
        if (episode + 1) % 200 == 0 or episode == 0:
            ...
            if test_accuracy > last_accuracy: # test one time per 200 episode, test 5 times, choose best acc
                # save networks
                ...
                OA = acc
                C = metrics.confusion_matrix(labels, best_predict)
                A[iDataSet, :] = np.diag(C) / np.sum(C, 1, dtype=np.float)

                k[iDataSet] = metrics.cohen_kappa_score(labels, best_predict)

            print('best episode:[{}], best accuracy={}'.format(best_episdoe + 1, last_accuracy))

    return last_accuracy, best_predict, G, RandPerm, Row, Column, nTrain, train_end,test_end
...
for iDataSet in range(nDataSet):
    ...
    # print('iter:{} best episode:[{}], best accuracy={}'.format(iDataSet, best_episdoe + 1, test_accuracy))
    print('***********************************************************************************')
...
classification_map(hsi_pic[16:-16, 16:-16, :], best_G[16:-16, 16:-16], 24, "classificationMap/IP_{}shot.png".format(TEST_SHOT_NUM_PER_CLASS+TEST_QUERY_NUM_PER_CLASS))
...
for i in range(GroundTruth.shape[0]):
    for j in range(GroundTruth.shape[1]):
        ...
        if GroundTruth[i][j] == 9:
            hsi_pic[i, j, :] = [0.75, 0.5, 0.75]
        if GroundTruth[i][j] == 10:
            hsi_pic[i, j, :] = [0.75, 1, 0.5]
        if GroundTruth[i][j] == 11:
            hsi_pic[i, j, :] = [0.5, 1, 0.65]
        if GroundTruth[i][j] == 12:
            hsi_pic[i, j, :] = [0.65, 0.65, 0]
        if GroundTruth[i][j] == 13:
            hsi_pic[i, j, :] = [0.75, 1, 0.65]
        if GroundTruth[i][j] == 14:
            hsi_pic[i, j, :] = [0, 0, 0.5]
        if GroundTruth[i][j] == 15:
            hsi_pic[i, j, :] = [0, 1, 0.75]
        if GroundTruth[i][j] == 16:
            hsi_pic[i, j, :] = [0.5, 0.75, 1]
classification_map(hsi_pic, GroundTruth, 24, "classificationMap/IP_yuan.png")
