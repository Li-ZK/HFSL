...
parser.add_argument("-d","--tar_input_dim",type = int, default = 144) # PaviaU=103；salinas=204； IN=200; KSC=176;Botswana = 144
...
parser.add_argument("-t","--test_episode", type = int, default = 1) #200
...
def load_data(image_file, label_file,label_file1):
    ...
    GroundTruth1 = label_data1[label_key1]
    ...
    return Data_Band_Scaler, GroundTruth,GroundTruth1  # image:(512,217,3),label:(512,217)
...
def get_alltest_loader(Data_Band_Scaler, GroundTruth,class_num ,shot_num_per_class):
        ...
        for i in range(m):
            indices = [j for j, x in enumerate(Row.ravel().tolist()) if
                       G[Row[j], Column[j]] == i + 1]  # G ndarray Row中的索引
            np.random.shuffle(indices)  #
        ...
        train_dataset = TensorDataset(data_tensor=trainX, target_tensor=trainY)
        train_loader = DataLoader(train_dataset, batch_size=class_num * shot_num_per_class, shuffle=False,
                                  num_workers=0)
        return train_loader,G, RandPerm, Row, Column,nTrain


test_Houston_data = './datasets/Houston/data.mat'
test_Houston_label = './datasets/Houston/mask_train.mat'
test_Houston_label1 = './datasets/Houston/mask_test.mat'

Data_Band_Scaler, GroundTruth,GroundTruth1 = load_data(test_Houston_data, test_Houston_label,test_Houston_label1)

def get_target_dataset(Data_Band_Scaler, GroundTruth):

    train_loader, _, imdb_da_train, _, _, _, _, _,nBand= get_train_test_loader(Data_Band_Scaler = Data_Band_Scaler, GroundTruth = GroundTruth, \
                        class_num=TEST_CLASS_NUM, shot_num_per_class=TEST_SHOT_NUM_PER_CLASS+TEST_QUERY_NUM_PER_CLASS) # 9类，每类5个有标记样本
    test_loader,G, RandPerm, Row, Column,nTrain = get_alltest_loader(Data_Band_Scaler = Data_Band_Scaler, GroundTruth = GroundTruth1, \
                        class_num=TEST_CLASS_NUM, shot_num_per_class=TEST_SHOT_NUM_PER_CLASS+TEST_QUERY_NUM_PER_CLASS)
...
torch.save(model.state_dict(), "VGG16-Houston.pth")  # 只保存模型的参数
...
class Network(nn.Module):
    def __init__(self, n_bands, n_classes):
        ...
        pretrained_dict = torch.load('./VGG16-Houston.pth')
        ...
        # model.conv.add_module("AdversarialNetwork", AdversarialNetwork(512, 512))
...
seeds = [1222,1239,1226,1337,1240,1994,1996,1117,1334, 1330]
seeds = [1222]	
def train():
    ...
    return test_accuracy, predict, G, RandPerm, Row, Column, nTrain, train_end,test_end

for iDataSet in range(nDataSet):
    np.random.seed(seeds[iDataSet])
    test_accuracy, predict,G, RandPerm, Row, Column, nTrain,train_end,test_end = train()
...
classification_map(hsi_pic[16:-16, 16:-16, :], best_G[16:-16, 16:-16], 24, "classificationMap/Houston_{}shot.png".format(TEST_SHOT_NUM_PER_CLASS+TEST_QUERY_NUM_PER_CLASS))

hsi_pic = np.zeros((GroundTruth.shape[0], GroundTruth.shape[1], 3))
for i in range(GroundTruth.shape[0]):
    for j in range(GroundTruth.shape[1]):
        if GroundTruth1[i][j] == 0:
            hsi_pic[i, j, :] = [0, 0, 0]
        if GroundTruth1[i][j] == 1:
            hsi_pic[i, j, :] = [0.77,0.87,0.7]
        if GroundTruth1[i][j] == 2:
            hsi_pic[i, j, :] = [0.43,0.67,0.27]
        if GroundTruth1[i][j] == 3:
            hsi_pic[i, j, :] = [0.32,0.50,0.2]
        if GroundTruth1[i][j] == 4:
            hsi_pic[i, j, :] = [0.21,0.34,0.13]
        if GroundTruth1[i][j] == 5:
            hsi_pic[i, j, :] = [0.77,0.35,0.06]
        if GroundTruth1[i][j] == 6:
            hsi_pic[i, j, :] = [0,0.69,0.94]
        if GroundTruth1[i][j] == 7:
            hsi_pic[i, j, :] = [0.75,0,0]
        if GroundTruth1[i][j] == 8:
            hsi_pic[i, j, :] = [0.7,0.78,0.9]
        if GroundTruth1[i][j] == 9:
            hsi_pic[i, j, :] = [0.48,0.48,0.48]
        if GroundTruth1[i][j] == 10:
            hsi_pic[i, j, :] = [0.95,0.69,0.51]
        if GroundTruth1[i][j] == 11:
            hsi_pic[i, j, :] = [0.97,0.79,0.67]
        if GroundTruth1[i][j] == 12:
            hsi_pic[i, j, :] = [0.34,0.34,0.34]
        if GroundTruth1[i][j] == 13:
            hsi_pic[i, j, :] = [0.8,0.8,0]
        if GroundTruth1[i][j] == 14:
            hsi_pic[i, j, :] = [0,0.8,0.4]
        if GroundTruth1[i][j] == 15:
            hsi_pic[i, j, :] = [1,0,0]

classification_map(hsi_pic, GroundTruth, 24, "classificationMap/Houston_yuan.png")
